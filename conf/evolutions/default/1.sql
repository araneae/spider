...
create table `message_box` (`message_box_id` BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY,`user_id` BIGINT NOT NULL,`message_box_type` INTEGER NOT NULL,`name` VARCHAR(254) NOT NULL);
create unique index `idx_unique_on_message_box_type` on `message_box` (`user_id`,`message_box_type`);
...
create table `user_message` (`user_id` BIGINT NOT NULL,`message_id` BIGINT NOT NULL,`message_box_id` BIGINT NOT NULL,`read` BOOLEAN DEFAULT false NOT NULL,`important` BOOLEAN DEFAULT false NOT NULL,`star` BOOLEAN DEFAULT false NOT NULL);
...
drop table `message`;
ALTER TABLE shared_document DROP PRIMARY KEY;
drop table `shared_document`;
drop table `skill`;
