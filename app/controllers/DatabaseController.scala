...
//                        Ok("")
  ...
  def searchDocument(documentId: Long, searchTerms: String) = IsAuthenticated{ username => implicit request =>
    //logger.info(s"in DatabaseController.searchDocument(${documentId}, ${searchTerms})")
    println(s"in DatabaseController.searchDocument(${documentId}, ${searchTerms})")
    ...
    if (searchTerms.length() > 0){
      ...
      val result = f.map{
           ...
           case _ => Ok(HttpResponseUtil.reponseEmptyObject())
//                case Failure(failure) =>
//                        println(s"Failrure ${failure}")
//                        Ok("")
      }
