...
class UserDocumentFolders(tag: Tag) extends Table[UserDocumentFolder](tag, "user_document_folder") {
  ...
  override def * = (userDocumentFolderId.?, documentFolderId, userId, ownershipType, canCopy, isLimitedShare, shareUntilEOD, createdUserId, createdAt, updatedUserId, updatedAt) <> (UserDocumentFolder.tupled, UserDocumentFolder.unapply)
  
  def ? = (userDocumentFolderId.?, documentFolderId.?, userId.?, ownershipType.?, canCopy.?, isLimitedShare.?, shareUntilEOD, createdUserId.?, createdAt.?, updatedUserId, updatedAt)
